...
class CardDesc:
    positive_kw = ["Trample", "Flying", "Lifelink", "Haste", "Reach", "Vigilance", "Deathtouch"]
    two_word_positive_kw = ["Add", "First"]
    ...
    @staticmethod
    def _compare_cost(cost1, cost2):
        if 'X' in cost1 + cost2:
            # TODO: support
            return Compare.Incomparable
        def decompose_cost(cost):
            ret = [0, 0, 0, 0, 0, 0]
            offs = 1
            ...
            ret[0] = sum(ret)
            return ret
        dc1 = decompose_cost(cost1)
        dc2 = decompose_cost(cost2)
        # Less cost is better 
    ...
    def compare(self, other):
        ...
        pt_result = CardDesc._compare_arrays([self.pow, self.tou], [other.pow, other.tou])
